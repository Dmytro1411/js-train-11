...
function customSet(arr) {
  ...
  for (let value of arr1) {
    if (typeof value === "number") {
      arr1.delete(value);
      //   log(arr1); // Данный log позволяет увидеть ход итерации
    }
  }
  return arr1;
}
...
function removeDuplicatesInPlace(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (uniquSet.has(arr[i])) {
      // Якщо елемент вже є унікальним, видаляємо його з масиву
      arr.splice(i, 1); // Если такой элемент уже есть, то удаляем его из массива
      // log(arr.splice(i, 1));
